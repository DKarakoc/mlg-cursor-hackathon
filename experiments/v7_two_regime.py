...
_memory = {"tick": -1, "phase": "NS_GREEN", "since": 0, "ema": 0.0, "heavy": False, "mode_age": 0}


def control(state):
    tick = state["tick"]
    if tick <= _memory["tick"]:
        _memory.update(phase="NS_GREEN", since=0, ema=0.0, heavy=False, mode_age=0)
    _memory["tick"] = tick
    ...
    def endgame_bonus(iid, direction, queue):
        if remaining >= ENDGAME_TICKS or queue == 0:
            return 0.0
        horizon = remaining - 6 * (hops_to_exit(iid, direction) - 1) - 6
        return SAVABLE_WEIGHT * max(0, min(queue, horizon))
    ...
    _memory["ema"] = _memory["ema"] * (1 - MODE_ALPHA) + MODE_ALPHA * per_intersection
    _memory["mode_age"] += 1
    if _memory["mode_age"] >= MODE_DWELL:
        if _memory["heavy"] and _memory["ema"] <= HEAVY_EXIT:
            _memory.update(heavy=False, mode_age=0)
        elif not _memory["heavy"] and _memory["ema"] >= HEAVY_ENTER:
    ...
    def movement_weight(iid, direction):
        ...
        return queue + INCOMING_GREEN * incoming + endgame_bonus(iid, direction, queue)
